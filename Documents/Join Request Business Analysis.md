...
Resolves whether the requester's email already owns an Identity, on approval. Referenced, not owned, by this document.
...
Self-serve Identity creation needs at least email verification, and probably rate limiting per address and per source. Neither is designed. This is the one open question that should be settled before the feature is exposed publicly rather than after.
